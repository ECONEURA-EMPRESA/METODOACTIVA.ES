...
async def chat_endpoint(request: ChatRequest, authorization: str = Header(None)):
    ...
    if authorization:
        try:
            ...
            logger.warning(f"⚠️ Auth Token Invalid, proceeding as Anonymous: {e}")
    else:
        logger.info("⚠️ No Token provided, proceeding as Anonymous")
    ...
    try:
        ...
        logger.exception(f"❌ Error Critical in Chat Endpoint: {e}")
