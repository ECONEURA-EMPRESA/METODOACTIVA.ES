...
const PremiumLoader = () => {
    ...
    useEffect(() => {
        ...
        const colors = ['#F7941D', '#EC008C', '#2DD6F5']; // Brand Colors
        ...
        const createParticle = () => {
            ...
            const left = Math.random() * 300 - 150; // Random X within center range
            const moveX = (Math.random() - 0.5) * 50; // Lateral drift
            const delay = Math.random() * 2;

            // Apply styles programmatically to avoid complex dynamic CSS
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-[radial-gradient(circle_at_center,_#fdfbf7_0%,_#f3f4f6_100%)] overflow-hidden font-sans">
            <style jsx>{`
                ...
                @keyframes shadowPulse {
                    0%, 100% { transform: rotateX(80deg) scale(1); opacity: 0.3; }
                    50% { transform: rotateX(80deg) scale(0.6); opacity: 0.1; }
                }
                @keyframes spinRing1 { from { transform: rotateX(60deg) rotateY(10deg) rotateZ(0deg); } to { transform: rotateX(60deg) rotateY(10deg) rotateZ(360deg); } }
                @keyframes spinRing2 { from { transform: rotateX(45deg) rotateY(-10deg) rotateZ(0deg); } to { transform: rotateX(45deg) rotateY(-10deg) rotateZ(360deg); } }
                @keyframes spinRing3 { from { transform: rotateX(70deg) rotateZ(0deg); } to { transform: rotateX(70deg) rotateZ(360deg); } }
                ...
                .logo-coin-wrapper {
                     ...
                     animation: coinSpin 8s cubic-bezier(0.4, 0, 0.2, 1) infinite;
                }
            `}</style>

            <div ref={containerRef} className="relative w-[400px] h-[400px] flex justify-center items-center perspective-[1200px]">
                ...
                {/* LOGO COIN (Center) */}
                {/* LOGO REMOVED - JUST PURE ENERGY CORE */}
